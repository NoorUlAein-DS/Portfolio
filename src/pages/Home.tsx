...
const Home: React.FC = () => {
  return (
    <div className="min-h-[calc(100vh-80px)] px-4 sm:px-6 py-8 md:py-12 flex flex-col items-center overflow-x-hidden">
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-6 border border-[#d4a5e0]/30 shadow-sm shadow-[#d4a5e0]/10">
            ...
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest dark:text-white text-gray-700">
              Noor Ul Aein Salamat Khan
            </span>
          </div>
          ...
          <p className="text-base sm:text-lg md:text-xl dark:text-gray-400 text-gray-600 mb-8 max-w-lg mx-auto lg:mx-0 leading-relaxed font-medium">
            Crafting data-driven solutions and building elegant digital experiences through a lens of extreme focus and technical rigor.
          </p>

          {/* Buttons */}
          <div className="flex flex-wrap justify-center lg:justify-start gap-4">
            <Link to="/projects" className="px-6 sm:px-8 py-3 sm:py-4 bg-[#C084FC] hover:bg-[#A855F7] text-white rounded-2xl font-bold transition-all hover:scale-105 shadow-xl shadow-[#C084FC]/20 flex items-center gap-2">
              View My Work <ArrowRight size={20} />
            </Link>
            <Link to="/contact" className="px-6 sm:px-8 py-3 sm:py-4 glass dark:text-white text-gray-600 rounded-2xl font-bold transition-all hover:bg-white/10 border border-[#C084FC]/20">
              Get In Touch
            </Link>
          </div>
        </motion.div>
        ...
        >
          <div className="relative w-48 h-48 sm:w-64 sm:h-64 md:w-80 md:h-80 xl:w-96 xl:h-96">
            <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-[#C084FC] to-[#E9D5FF] blur-3xl opacity-30 animate-pulse" />
          </div>
        </motion.div>
      </div>

      {/* Skills Section */}
      <div className="w-full max-w-7xl mt-16 sm:mt-24">
        <h2 className="text-2xl sm:text-3xl font-bold mb-8 dark:text-white text-gray-700 text-center">My Skill Progress</h2>
        <SkillCards /> 
      </div>

      {/* Quick Actions / Cards */}
      <div className="w-full max-w-6xl mt-16 sm:mt-24">
        <h2 className="text-2xl sm:text-3xl font-bold mb-8 dark:text-white text-gray-700 text-center lg:text-left">What I Build</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {QUICK_ACTIONS.map((action, idx) => (
            <motion.div
              key={action.title}
              ...
              transition={{ delay: idx * 0.1 }}
              whileHover={{ y: -5 }}
              className="glass p-6 sm:p-8 rounded-3xl border border-white/10 flex flex-col items-center text-center group cursor-default shadow-lg"
            >
              <div className={`${action.color === 'bg-purple-500' ? 'bg-[#C084FC]' : action.color} p-4 sm:p-5 rounded-2xl text-white mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all`}>
                {action.icon}
              </div>
              <h3 className="font-bold text-lg sm:text-xl dark:text-white text-gray-700 mb-2 break-words w-full">
                {action.title}
              </h3>
